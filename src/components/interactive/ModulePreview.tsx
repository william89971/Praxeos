...
/**
 * Small Canvas 2D animated preview for module cards on the homepage.
 *
 ...
 */
export function ModulePreview({ variant, width = 240, height = 130 }: Props) {
    ...
    const draw = (now: number) => {
      ...
      switch (variant) {
        case "monetary-garden":
          drawMonetaryGarden(ctx, t, width, height, palette);
          break;
        case "signal-orchard":
          drawSignalOrchard(ctx, t, width, height, palette);
          break;
        case "calculation-labyrinth":
          drawCalculationLabyrinth(ctx, t, width, height, palette);
          break;
        case "coordination-engine":
          drawCoordinationEngine(ctx, t, width, height, palette);
          break;
      }
...
function drawSignalOrchard(
  ...
  for (let i = 0; i < positions.length; i++) {
    ...
    ctx.beginPath();
    ctx.arc(pos.x, pos.y, 2 + intensity * 1.5, 0, Math.PI * 2);
    ctx.fill();
  }
  ctx.globalAlpha = 1;
}
...
function drawCoordinationEngine(
  ...
  for (let r = 0; r < radii.length; r++) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      ctx.beginPath();
      ctx.arc(x, y, 2 + pulse * 0.6, 0, Math.PI * 2);
      ctx.fill();
    }
  }
  ctx.globalAlpha = 1;
