...
export async function checkGuideRateLimit(networkId: string): Promise<RateLimitResult> {
  ...
  const minute = new Ratelimit({ redis, limiter: Ratelimit.slidingWindow(6, "1 m"), prefix: "guide:min" });
  const day = new Ratelimit({ redis, limiter: Ratelimit.slidingWindow(30, "1 d"), prefix: "guide:day" });
  const [minuteResult, dayResult] = await Promise.all([minute.limit(id), day.limit(id)]);
  if (minuteResult.success && dayResult.success) return { success: true, retryAfterSeconds: 0 };
