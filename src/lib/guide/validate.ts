...
export function validateGuideTurn(turn: GuideTurn): { ok: true } | { ok: false; reason: string } {
  ...
  if (turn.blocks.length === 0) return { ok: false, reason: "Guide needs an explanation block." };
  const uncitedClaim = turn.blocks.some(
    (block) => block.category !== "observation" && block.text.trim().length > 0 && block.citations.length === 0,
  );
  if (uncitedClaim) return { ok: false, reason: "Every factual concept explanation must be cited." };
  return { ok: true };
}
