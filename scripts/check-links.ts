...
async function main() {
  if (baseUrl) {
    await Promise.all(
      ROUTES.map((route) => assertReachable(new URL(route, baseUrl).href)),
    );
  }
  ...
  try {
    ...
    console.log(
      `Checked ${ROUTES.length} internal route contracts and ${urls.length} source links.`,
    );
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
    console.log(
      `Checked ${ROUTES.length} internal route contracts; source packets not added yet.`,
    );
  }
}
